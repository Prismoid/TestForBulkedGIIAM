...
/**
   * 自作関数
   * TLDc, SLDc: 56bit, range: 32bit * 2 = 64bit 
   * 探索
   */

var BigNumber = require('bignumber.js'); // npm install bignumber, https://www.npmjs.com/package/bignumber.js
AVLTree.prototype.find64 = function find64 (key){		       
    // 必要な変数
    var root = this._root;
    var div64 = new BigNumber("0x10000000000000000", 16);
    var div32 = new BigNumber("0x100000000", 16);
    // key: 56 + 32 = 88bit
    var keyIDSpace = key.divToInt(div32); // 56bit
    var keyTarget = key.mod(div32); // 32bit
    // subtree.key: 56 + 64 = 120bit
    var tmpKey; // 120bit
    var tmpKeyIDSpace; // 56bit
    var tmpStartOfRange; // 32bit
    var tmpEndOfRange; // 32bit
    // Tree Dataと比較とフラグ変数
    var subtree = root, cmp;
    var compare = this._comparator;
    var flag = 0; // フラグ変数
    while (subtree) {
	console.log("HEY! by Find64-1");
	tmpKey = subtree.key; // 120bit
	tmpKeyIDSpace = tmpKey.divToInt(div64); // 56bit
	cmp = compare(keyIDSpace, tmpKeyIDSpace);
	if (flag === 0) {
	    console.log("HEY! by Find64-2");
	    if (cmp === 0) {
		flag = 1;
		console.log("HEY! by Find64-3");
	    } else if (cmp < 0) {
	...
	tmpStartOfRange = (tmpKey.mod(div64)).divToInt(div32); // 32bit
	tmpEndOfRange = tmpKey.mod(div32); // 32bit
	// flag = 1が代入されるとすぐ実行される
	if (flag === 1) {
	    console.log("HEY! by Find64-4");
	    if ((tmpStartOfRange.lte(keyTarget)) && (keyTarget.lte(tmpEndOfRange))) {
		console.log("HEY! by Find64-5");
		return subtree;
	    } else if (keyTarget.lt(tmpStartOfRange)) {
		return subtree.left;
	    } else {
